...
\author{\textbf{Milad Awad} \textit{Hochschule Bremen -- Matrikelnr. 5358834 -- mawad@stud.hs-bremen.de}%
	\\Supervisor: M.\,Sc.\,Tuncer Catalkaya%
	\vspace{-0.5cm}
}
...
\begin{abstract}
	...
	Flutter offer significant cost reductions, concerns regarding performance overhead persist. This article
...
\textit{To what extent does the performance overhead of state-of-the-art cross-platform frameworks (Flutter)
...
The remainder of this article is organized as follows. Section~II introduces the technical background on native and cross-platform development. Section~III reviews the related work on cross-platform frameworks, performance benchmarking, and resource efficiency. Section~IV describes the experimental design, the benchmark application, and the test environment. Section~V presents the quantitative results across all metrics. Section~VI discusses the interpretation of the findings and the limitations of this study. Finally, Section~VII concludes the article with a summary and practical recommendations.

\section{Background}
This section introduces the key technologies and concepts that underpin the comparative analysis presented in this article.

\subsection{Native iOS Development}
Native iOS applications are developed using Apple's proprietary toolchain. Swift~\cite{apple2024swift}, Apple's modern programming language, is compiled via the LLVM compiler infrastructure, which applies aggressive optimizations such as auto-vectorization, loop unrolling, and stack-based memory allocation for primitive types. The user interface layer is built with SwiftUI~\cite{apple2024swiftui}, a declarative framework introduced in 2019 that constructs a view hierarchy managed by the UIKit runtime. Rendering is performed through Core Animation, which composites layers on the GPU via Apple's Metal graphics API.

\subsection{Flutter and Dart}
Flutter is Google's open-source UI toolkit for building natively compiled applications from a single codebase~\cite{flutter2024}. It uses Dart~\cite{dart2024} as its programming language, which supports both Just-in-Time (JIT) compilation for development and Ahead-of-Time (AOT) compilation for production builds. Unlike hybrid frameworks that rely on platform UI components or web views, Flutter renders every pixel directly through its own rendering engine, bypassing the host platform's UI toolkit entirely~\cite{bjornhansen2018}.

\subsection{The Impeller Rendering Engine}
Impeller is Flutter's modern rendering backend, introduced to replace the Skia-based engine~\cite{impeller2024}. Its key architectural innovation is the pre-compilation of all shader programs at build time, which eliminates the runtime shader compilation pauses (commonly known as ``shader jank'') that plagued the previous Skia backend. On iOS, Impeller renders directly through Apple's Metal API, producing a pipeline conceptually similar to---but independent from---the native Core Animation stack.

\subsection{Key Metrics}
The following metrics are used throughout this article to quantify performance differences:
\begin{itemize}
	\item \textbf{CPU Efficiency}: The wall-clock execution time of a deterministic algorithm on the main thread, measured in milliseconds (ms).
	\item \textbf{Rendering Stability}: The average frames per second (FPS) and the Jank rate, defined as the percentage of frames exceeding a 25~ms threshold ($1.5\times$ the 16.67~ms target at 60~Hz).
	\item \textbf{Engine Initialization Time}: The interval from the framework's initialization hook to the first rasterized frame, excluding OS-level process creation overhead.
\end{itemize}
...
literature regarding real-world performance on Apple silicon, which this article addresses.
...
struggle to match the CPU performance of native code. This article extends that inquiry by investigating
...
\begin{itemize}
	...
	\item Measurement: Execution time is measured in milliseconds (ms) using \texttt{Stopwatch} (Dart) and \texttt{CFAbsolute\-Time\-Get\-Current} (Swift).
\end{itemize}
...
applications (Native and Flutter) has been made open-source. The repository also includes detailed technical documentation, screenshots of the benchmark application, and demo videos illustrating the test procedures. The repository is available at:
\\
\url{https://github.com/Milad9A/WISA-iOS-Benchmark}

\subsection{Use of AI Tools}
In accordance with academic transparency requirements, I disclose that AI-based language models were used during the preparation of this article for editorial assistance, including grammar checking, phrasing suggestions, and code review. All generated content was critically reviewed, verified, and adapted by me. The scientific analysis, experimental design, data collection, and interpretation of results are entirely my own work.
...
Analysis: Contrary to historical assumptions, Flutter slightly outperformed the native solution in frame stability (Fig.~\ref{fig:gpu}). This is attributed to the Impeller engine~\cite{impeller2024}, which pre-compiles all shaders at build time, eliminating runtime compilation hitches. Flutter bypasses Core Animation and rasterizes its widget tree directly via Metal, resulting in more predictable frame pacing. SwiftUI exhibited minor volatility (max frame time of approximately 77~ms), likely due to main-thread layout passes during rapid scrolling.
...
(Fig.~\ref{fig:init}). The pre-warmed Flutter engine, combined with Dart's AOT-compiled machine code, constructs its initial widget tree more rapidly than SwiftUI can resolve its declarative view hierarchy. Note that this metric excludes OS-level cold-start overhead (process creation, dynamic library loading), which typically adds 100--300~ms on both platforms.
...
\begin{itemize}
	\item Memory (RAM): As shown in Fig.~\ref{fig:memory}, Native iOS was significantly more efficient, consuming approximately 18~MB at idle compared
...
\section{Discussion}
This section interprets the empirical results presented in Section~V and discusses their practical implications for mobile architecture decisions, followed by the limitations of this study.
...
\section{Conclusion}
This article presented a comparative analysis of Native iOS and Flutter development on physical iPhone~14
...
I hereby declare that this article is entirely my own work and that I have not used any sources or aids other than those listed. All passages taken verbatim or in substance from published or unpublished sources have been identified as such.

\vspace{1cm}
\noindent Bremen, February 2026 \hfill Milad Awad
